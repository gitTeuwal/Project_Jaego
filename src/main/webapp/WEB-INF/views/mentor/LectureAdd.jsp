...
<form method="POST" enctype="multipart/form-data">
	 	...
	 	<input type="hidden" name="memberId" value="${sessionScope.memberLogin.memberId}"/> <!-- 나중에 세션이나 모델을 통해 멘토 아이디를 가져가야하므로 바꿔야함 -->
			...
			<div class="mb-3 mt-3">
	 	    ...
	 	    <div class="mb-3 mt-3">
    			<h4>클래스 기간</h4>
					시작 날짜<br> <p> <input type="date" name="lectureStart" required/> </p> 끝나는 날짜 <br>  <p><input type="date" name="lectureEnd" required/> </p>
			</div>	
	 	    <div class="mb-3 mt-3">
		   ...
		   <br>
					<h4>시간 당 가격</h4>
					<input type='text' name="lecturePrice" placeholder="시간당 가격 (₩)" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">	(₩)원		
			</div>
			<div class="mb-3 mt-3">
			  <br>
				<h4>클래스 커버 이미지</h4> 
					<img alt="임의 사진" src="#"  width="800" height="400"  class="mx-auto mb-2 d-block " onerror="this.src='./resources/icon/logo.jpg'" id="preview" />			
					<input type="file" name="lectureImg1" accept="image/jpg,image/jpeg,image/png" class="form-control mt-3" id="imgSelector"  >
			</div>	
			<div class="mb-3 mt-3">
		   <br>
					<h4>클래스 커리큘럼 이미지</h4> 
					<img alt="임의 사진" src="#"  width="800" height="400"  class="mx-auto mb-2 d-block " onerror="this.src='./resources/icon/logo.jpg'" id="preview1"  />			
